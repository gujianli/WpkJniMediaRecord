...
static void x264_frame_dump( x264_t *h )
{
    ...
    cw = h->param.i_width>>1;
    ch = h->param.i_height>>1;
    planeu = (pixel*)x264_malloc( (cw*ch*2+32)*sizeof(pixel) );
    planev = planeu + cw*ch + 16;
...
static int x264_validate_parameters( x264_t *h )
{
    ...
    max_slices = (h->param.i_height+15)/16;
    if( h->param.b_sliced_threads )
    ...
    if( !h->param.analyse.i_trellis )
        h->param.analyse.f_psy_trellis = 0;
    h->param.analyse.f_psy_rd = x264_clip3ff( h->param.analyse.f_psy_rd, 0, 10 );
    h->param.analyse.f_psy_trellis = x264_clip3ff( h->param.analyse.f_psy_trellis, 0, 10 );
    ...
    h->param.rc.f_aq_strength = x264_clip3ff( h->param.rc.f_aq_strength, 0, 3 );
...
{
    int satd = h->param.analyse.i_subpel_refine > 1;
...
x264_t *x264_encoder_open( x264_param_t *param )
{
    ...
    x264_quant_init( h->param.cpu, &h->quantf );
...
#undef COPY
    ...
    ret = x264_validate_parameters( h );
...
#define _MCDULL_ENCODER_
#include "dull_encoder.c"
